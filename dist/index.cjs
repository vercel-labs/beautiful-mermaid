...
var dagre = require('@dagrejs/dagre/dist/dagre.js');
...
function centerZBends(points, verticalFirst = true) {
...
function preComputeSubgraphLayout(sg, graph, opts) {
  const subG = new dagre__default.default.graphlib.Graph({ directed: true, compound: true });
  subG.setGraph({
  ...
  dagre__default.default.layout(subG);
...
async function layoutGraph(graph, options = {}) {
  ...
  const g = new dagre__default.default.graphlib.Graph({ directed: true, compound: true });
  g.setGraph({
...
function estimateNodeSize(id, label, shape, fontSize = FONT_SIZES.nodeLabel, fontWeight = FONT_WEIGHTS.nodeLabel, padX = NODE_PADDING.horizontal, padY = NODE_PADDING.vertical) {
...
function addSubgraphToDagre(g, sg, graph, opts, parentId) {
...
function extractPositionedGraph(g, graph, padding, preComputed, groupPaddingX = 16, groupPaddingY = 12) {
  const nodes = [];
  const groups = [];
  ...
  const extraPadX = Math.max(0, groupPaddingX - 16);
  const extraPadY = Math.max(0, groupPaddingY - 12);
...
function assignGroupRanks(groups, nodeRankMap, allNodes) {
  for (const group of groups) {
    ...
    for (const node of allNodes) {
      ...
      if (node.x >= group.x && node.x + node.width <= group.x + group.width && node.y >= group.y && node.y + node.height <= group.y + group.height) {
...
function resolveAnimation(animate) {
  if (!animate) return null;
  if (animate === true) return { ...DEFAULTS3 };
  return { ...DEFAULTS3, ...animate };
}
var NAMED_EASINGS = {
...
function cssEasingToSmil(easing) {
...
function computeDelays(graph, opts) {
...
var GROUP_REVEAL_PROGRESS = 0.6;
function collectGroupDelays(groups, allNodes, opts, nodeDelays, out) {
  for (const group of groups) {
...
function buildAnimationCSS(opts) {
  const nodeKeyframe = opts.nodeAnimation === "fade-up" ? "a-fade-up" : opts.nodeAnimation === "scale" ? "a-scale" : "a-fade";
  return `
  /* Animation keyframes */
...
// src/renderer.ts
function renderSvg(graph, colors, font = "Geist", transparent = false, options = {}, anim = null) {
...
function renderAnimatedEdge(edge, edgeIndex, lineWidth, bendRadius, delay, anim) {
...
function arrowMarkerDefs() {
  ...
  return `  <marker id="arrowhead" markerWidth="${w}" markerHeight="${h}" refX="0" refY="${h / 2}" orient="auto" markerUnits="userSpaceOnUse" overflow="visible">
...
function renderGroupBackground(group, groupCornerRadius = 0, groupBorderColor = "var(--_node-stroke)") {
  const parts = [];
  parts.push(renderGroupBackgroundInner(group, groupCornerRadius, groupBorderColor));
  for (const child of group.children) {
...
function renderGroupBackgroundInner(group, groupCornerRadius, groupBorderColor) {
  return `<rect x="${group.x}" y="${group.y}" width="${group.width}" height="${group.height}" rx="${groupCornerRadius}" ry="${groupCornerRadius}" fill="url(#diag-hatch)" stroke="${groupBorderColor}" stroke-width="${STROKE_WIDTHS.outerBox}" />`;
}
function renderGroupLabels(group, font, groupFontSize, groupFontWeight, groupFont, groupTextTransform) {
  const parts = [];
  parts.push(renderGroupLabelsInner(group, font, groupFontSize, groupFontWeight, groupFont, groupTextTransform));
  for (const child of group.children) {
...
function renderGroupLabelsInner(group, font, groupFontSize, groupFontWeight, groupFont, groupTextTransform) {
  ...
  return `<rect x="${bgX}" y="${bgY}" width="${bgW}" height="${bgH}" fill="var(--bg)" rx="2" ry="2" />
<text x="${group.x + 16}" y="${group.y + headerHeight / 2}" dy="${TEXT_BASELINE_SHIFT}" style="font-family:'${groupFont}',system-ui,sans-serif;font-size:${groupFontSize}px;font-weight:${groupFontWeight};${ttStyle}" fill="rgba(255,255,255,0.92)">${label}</text>`;
}
function renderEdge(edge, lineWidth, bendRadius) {
  if (edge.points.length < 2) return "";
  ...
  if (bendRadius > 0 && pts.length > 2) {
    ...
    return `<path d="${d}" fill="none" stroke="var(--_line)" stroke-width="${strokeWidth}"${dashArray}${markers} />`;
  }
...
function renderEdgeLabel(edge, font, edgeFontSize) {
...
<text x="${mid.x}" y="${mid.y}" text-anchor="middle" dy="${TEXT_BASELINE_SHIFT}" font-size="${edgeFontSize}" font-weight="${FONT_WEIGHTS.edgeLabel}" fill="var(--_text-muted)">${escapeXml(label)}</text>`;
}
...
function renderNodeShape(node, cr) {
  ...
  switch (shape) {
    ...
    case "rectangle":
    default:
      return renderRect(x, y, width, height, fill, stroke, sw, cr);
  }
}
function renderRect(x, y, w, h, fill, stroke, sw, cr = 0) {
...
function renderNodeLabel(node, font, fontSize, fontWeight, letterSpacing) {
  ...
  return `<text x="${cx}" y="${cy}" text-anchor="middle" dy="${TEXT_BASELINE_SHIFT}" font-size="${fontSize}" font-weight="${fontWeight}"${lsAttr} fill="${textColor}">${escapeXml(node.label)}</text>`;
}
...
async function layoutClassDiagram(diagram, _options = {}) {
  ...
  const g = new dagre__default.default.graphlib.Graph({ directed: true });
  g.setGraph({
    rankdir: "TB",
    acyclicer: "greedy",
...
exports.fromShikiTheme = fromShikiTheme;
exports.parseMermaid = parseMermaid;
exports.renderMermaid = renderMermaid;
exports.renderMermaidAscii = renderMermaidAscii;
